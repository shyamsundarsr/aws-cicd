...
# Wires all modules together. Resource dependencies flow:
#   vpc → security_groups → alb, rds → iam → ecs → github_oidc, route53
#
# `depends_on` on the ecs module ensures all ALB listeners and IAM roles are
# fully provisioned before ECS services attempt to register with the ALB.
#------------------------------------------------------------------------------
